    ...
    private static BiConsumer<Integer, Obj> desequip() {
        return (player, obj) -> {
            ...
            if (obj instanceof WeaponObj) {
                entity.removeWeapon();
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[0], new Class[] {Weapon.class}));
            } else if (obj instanceof ArmorObj) {
                entity.bodyIndex(((ArmorObj) obj).getBodyNumber()); // TODO change to raza body
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[]{entity.getBody()}, new Class[0]));
            } else if (obj instanceof HelmetObj) {
                entity.removeHelmet();
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[0], new Class[] {Helmet.class}));
            } else if (obj instanceof ShieldObj) {
                entity.removeShield();
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[0], new Class[] {Shield.class}));
            }
        };
    }
    ...
    private static BiConsumer<Integer, Obj> equip() {
        return (player, obj) -> {
            ...
            if (obj instanceof WeaponObj) {
                ...
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[]{entity.getWeapon()}, new Class[0]));
            } else if (obj instanceof ArmorObj) {
                entity.bodyIndex(((ArmorObj) obj).getBodyNumber());
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[]{entity.getBody()}, new Class[0]));
            } else if (obj instanceof HelmetObj) {
                entity.helmetIndex(((HelmetObj) obj).getAnimationId());
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[]{entity.getHelmet()}, new Class[0]));
            } else if (obj instanceof ShieldObj) {
                entity.shieldIndex(((ShieldObj) obj).getAnimationId());
                WorldManager.notifyUpdate(new EntityUpdate(player, new Component[]{entity.getShield()}, new Class[0]));
            }
        };
    }
}
